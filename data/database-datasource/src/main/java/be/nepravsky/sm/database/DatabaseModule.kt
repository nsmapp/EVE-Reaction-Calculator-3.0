...
class DatabaseModule {
    ...
    fun provideSqlDriver(context: Context): SqlDriver {
        val database = context.getDatabasePath("reactionsv412.db")

        if (!database.exists()) {
            val inputStream = context.assets.open("reactionsv41.db")
    ...
    @Single
    fun provideProjectItemsDao(database: Database): ProjectItemsTableQueries =
        database.projectItemsTableQueries
}
